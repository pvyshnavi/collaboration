(function () {
    ...
    function UserService($http) {
        ...
        function GetAll() {
            return $http.get('http://localhost:8085/CollobrationPortal/customers/').then(handleSuccess, handleError('Error getting all users'));
        }
 
        function GetById(id) {
            return $http.get('http://localhost:8085/CollobrationPortal/customers/' + id).then(handleSuccess, handleError('Error getting user by id'));
        }
        ...
        function Create(user) {
            return $http.post('http://localhost:8085/CollobrationPortal/customers/', user).then(handleSuccess, handleError('Error creating user'));
        }
